...
import { QUERY_ME, QUERY_USER_BATTLES, QUERY_USER_COLLECTIONS } from 'utils/queries/userQueries'
import { DELETE_BATTLE, ADD_BATTLE_TO_COLLECTION } from 'utils/mutations/battleMutations'
...
const BattleSelect = () => {
  ...
  // Add a battle
  const [addBattleToCollection, { error: mutation_error }] = useMutation(ADD_BATTLE_TO_COLLECTION, {
    refetchQueries: [{ query: QUERY_USER_COLLECTIONS }, 'UserCollections']
  })

  // Delete a battle
  const [deleteBattle] = useMutation(DELETE_BATTLE, {
    refetchQueries: [{ query: QUERY_USER_BATTLES }, 'UserBattles', { query: QUERY_USER_COLLECTIONS }, 'UserCollections']
  })
  ...
  const handleDrop = async (e, collectionId) => {
    ...
    try {
      ...
      console.error('💥 Battle was not added to Collection! \n', mutation_error)
    }
  }
  ...
  return (
    <>
      <PageHeader image={`url(${require('assets/images/card_backs/back_7.jpg')})`} pageTitle={'Saved Battles'} />
        ...
        <div className="deck-container">
          {collections &&
            collections.map(collection => {
              return <Deck key={collection._id} collection={collection} handleDrop={handleDrop} />
            })}

          {/* Add new collection button */}
          <figure className="add-deck m-2" onClick={() => setShowCollectionModal(true)}>
            <div className="plus-button h-100 d-flex justify-content-center align-items-center">+</div>
          </figure>
        </div>
      </section>
      ...
      <NewCollectionModal showModal={showCollectionModal} handleHide={handleCloseCollectionModal} userId={user._id} />
    </>
  )
}
