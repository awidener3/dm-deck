...
const Actions = ({ monster, handleRollDice, sortedData, setSortedData }) => {
  ...
  */
    ...
    actions.map((action) => {
      ...
      if (action.desc.includes("Melee or Ranged Weapon Attack")) {
        ...
        if (strArr.includes("DC"))
          ...
          } ${strArr[strArr.indexOf("DC") + 2].slice(0, 3).toUpperCase()}`;
      } else {
        console.log("other");
      }
  ...
  const [actions, setActions] = useState(() => processActions(monster.actions));
  ...
  return actions.map((action) => (
    ...
    >
      ...
      <div>
        {action.attack_type === "Melee" || action.attack_type === "Ranged" ? (
          <MeleeRangeButtons
            action={action}
