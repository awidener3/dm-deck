...
const BattleBuilder = () => {
  ...
  const { loading: battle_loading, error: query_battle_error } = useQuery(QUERY_BATTLE, {
    variables: { battleId },
  ...
  const handleSelectHero = id => {
    data.userCharacters.forEach(hero => {
      if (hero._id === id && !selectedHeroes.includes(hero)) setSelectedHeroes([...selectedHeroes, hero])
    })
  }
